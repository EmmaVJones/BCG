...
# Bring in Lou's attributes by Basin
att <- read.csv('C:/HardDriveBackup/R/BCG/Jason/finaltaxa_abovefalls_EVJ.csv')%>%
  mutate(lCommonName=tolower(CommonName))%>% #make all common names lowercase
  select(-c(BCG.Attribute,Num.Samples,Pct.Samples,CommonName,Phylosort,Notes))%>%#remove old CommonName column
  plyr::rename(c('lCommonName'='CommonName'))%>% 
  tidyr::gather(Subbasin_short,attLevel,-ScientificName,-CommonName)



## Bring in SampleList with lat/long data
sampleList <- read.csv('C:/HardDriveBackup/R/BCG/FiguringItOut/taxaLists/sampleList_GIS.csv',row.names=NULL)
...
SubBresults <- SubbasinConnection(sampleList)

# I want to keep Subbasin selection component separate from the actual model to give people a 'bail out' point
# in the shiny app if they notice an error in lat/long or in the shapefile itself
dfInCorrectFormat <- SubBresults
  ...
  for(i in 1:length(splits)){ #loop through each dataframe in the list of dataframes and do:
    ...
    if(splits[[i]]$Subbasin_short[1] %in% c('UNew','MNew_WV','MNew_VA','LNew','Greenbrier','Gauley')){
      if(splits[[i]]$Catchment[1] < 5){
        ...
        }else(result <- rbind(result,cbind(AboveFallsSmallModel_NOBKT(samplename,sampleathand),Model='AboveFalls,Small,NOBKT',Comment=comment1)))
        ...
        result <- rbind(result,cbind(AboveFallsSmallModel(samplename,sampleathand),Model='AboveFalls,Small',Comment=comment1)
                       ,cbind(AboveFallsMedLargeModel(samplename,sampleathand),Model='AboveFalls,MediumLarge',Comment=comment1))
                         ...
                         ,cbind(AboveFallsMedLargeModel_NOBKT(samplename,sampleathand),Model='AboveFalls,MediumLarge,NOBKT',Comment=comment1)))
        ...
        }else(result <- rbind(result,cbind(AboveFallsMedLargeModel_NOBKT(samplename,sampleathand),Model='AboveFalls,MediumLarge,NOBKT',Comment=comment1)))
      }
    }else{
      if(splits[[i]]$Catchment[1] < 5){
        if(any(splits[[i]]$CommonName %in% c('brook trout'))){
        result <- rbind(result,cbind(OtherSmallModel(samplename,sampleathand),Model='Other,Small',Comment=comment1))
        }else(result <- rbind(result,cbind(OtherSmallModel_NOBKT(samplename,sampleathand),Model='Other,Small,NOBKT',Comment=comment1)))
      }else if(splits[[i]]$Catchment[1] > 5 & splits[[i]]$Catchment[1] < 15){
        if(any(splits[[i]]$CommonName %in% c('brook trout'))){
        result <- rbind(result,cbind(OtherSmallModel(samplename,sampleathand),Model='Other,Small',Comment=comment1)
                       ,cbind(OtherMedLargeModel(samplename,sampleathand),Model='Other,MediumLarge',Comment=comment1))
        }else(result <- rbind(result,cbind(OtherSmallModel_NOBKT(samplename,sampleathand),Model='Other,Small,NOBKT',Comment=comment1)
                             ,cbind(OtherMedLargeModel_NOBKT(samplename,sampleathand),Model='Other,MediumLarge,NOBKT',Comment=comment1)))
      }else if(splits[[i]]$Catchment[1] > 15){
        if(any(splits[[i]]$CommonName %in% c('brook trout'))){
        result <- rbind(result,cbind(OtherMedLargeModel(samplename,sampleathand),Model='Other,MediumLarge',Comment=comment1))
        }else(result <- rbind(result,cbind(OtherMedLargeModel_NOBKT(samplename,sampleathand),Model='Other,MediumLarge,NOBKT',Comment=comment1)))
      }
    }
...
BCG_GIS <- BCG_Model_GIS(SubBresults)

# errors that come up are associated with no subbasin with attribute info
# Can fix when get finalized taxa by basin data
# next step: shiny App!

# test with Lou sites
hw1 <- as.data.frame(read_excel('C:/HardDriveBackup/R/BCG/Lou/abovefalls_hw1.xlsx',sheet='Sheet1'))%>%
  mutate(lCommonName=tolower(CommonName))%>% #make all common names lowercase
  select(c(ExerciseID,DrArea_mi2,Longitude,Latitude,lCommonName,ScientificName,Count)) %>% #get just the variables I need
  plyr::rename(c("ExerciseID"="SampleName","DrArea_mi2"="Catchment"
                 ,"lCommonName"="CommonName")) 

SubBresults <- SubbasinConnection(hw1)
BCG_GIS <- BCG_Model_GIS(SubBresults)
