source('global.R')
template <- read.csv('www/template.csv')
options(digits=3)
...
shinyServer(function(input, output, session) {
  ...
  # Download BCG model results
  output$downloadResults <- downloadHandler(filename=function(){paste('BCGResults_',format(Sys.time(),"%m%d%Y"),'.csv',sep="")},
                                                 content=function(file){write.csv(BCGresults(),file,row.names=F)})
